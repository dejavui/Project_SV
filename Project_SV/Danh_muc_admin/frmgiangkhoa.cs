...
{
    public partial class frmgiangkhoa : Form
    {
        public frmgiangkhoa()
        {
            InitializeComponent();
        ...
        private void loadgr()
        {
            string giangkhoa = "select giangkhoa.ID, chuongtrinh.ten_ct, khoa.ten_khoa,monhoc.ten_mh, giangkhoa.nam_hoc,giangkhoa.hoc_ky,giangkhoa.so_tiet_ly_thuyet,giangkhoa.so_tiet_thuc_hanh,giangkhoa.so_tin_chi from chuongtrinh inner join giangkhoa on chuongtrinh.ma_ct = giangkhoa.ma_ct inner join khoa on khoa.ma_khoa = giangkhoa.ma_khoa inner join monhoc on monhoc.ma_mh = giangkhoa.ma_mh";
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            ...
            if (txtnamhoc.Text.Trim() == "" || txthocky.Text.Trim() == "" || txtlyhtuyet.Text.Trim() == "" || txtthuchanh.Text.Trim() == "" || txttinchi.Text.Trim() == "")
            {
                MessageBox.Show("Bạn nhập thiếu rồi !!");
                return;
            }
            string sql1 = "select * from giangkhoa where ID=N'" + txtid.Text.Trim() + "'";
            kn.kt(sql1);
            ...
            if (flag == true)
            {
                try
                {
                string sqlthem = "insert into giangkhoa(ID,ma_ct,ma_khoa,ma_mh,nam_hoc,hoc_ky,so_tiet_ly_thuyet,so_tiet_thuc_hanh,so_tin_chi) values('"+txtid.Text.Trim()+"', '"+cbchuongtrinh.SelectedValue.ToString()+"', '"+cbkhoa.SelectedValue.ToString()+"', '"+cbmonhoc.SelectedValue.ToString()+"', '"+txtnamhoc.Text.Trim()+"', '"+txthocky.Text.Trim()+"', '"+txtlyhtuyet.Text.Trim()+"', '"+txtthuchanh.Text.Trim()+"', '"+txttinchi.Text.Trim()+"')";
                kn.sqlquery(sqlthem);
                loadgr();
                loaddata();
                }
        ...
        private void timkiem()
        {
            ...
            {
            string timkiem = "select giangkhoa.ID, chuongtrinh.ten_ct, khoa.ten_khoa,monhoc.ten_mh, giangkhoa.nam_hoc,giangkhoa.hoc_ky,giangkhoa.so_tiet_ly_thuyet,giangkhoa.so_tiet_thuc_hanh,giangkhoa.so_tin_chi from chuongtrinh inner join giangkhoa on chuongtrinh.ma_ct = giangkhoa.ma_ct inner join khoa on khoa.ma_khoa = giangkhoa.ma_khoa inner join monhoc on monhoc.ma_mh = giangkhoa.ma_mh where ID like'%"+txttimkiem.Text.Trim()+"%' ,ten_ct like '%" + txttimkiem.Text.Trim()+"%' or ten_mh like '%"+txttimkiem.Text.Trim()+"%' or ten_khoa like '%"+txttimkiem.Text.Trim()+ "%' or chuongtrinh.ma_ct like '%"+txttimkiem.Text.Trim()+"%' or khoa.ma_khoa like '%"+txttimkiem.Text.Trim()+"%' or monhoc.ma_mh like '%"+txttimkiem.Text.Trim()+"%'";
            kn.sqlquery(timkiem);
        ...
        private void Xóa_Click(object sender, EventArgs e)
        {
            ...
            if (traloi == DialogResult.Yes)
            {
                ...
                string sqlxoa = "delete giangkhoa where ID ='" + ma + "'";
                kn.sqlquery(sqlxoa);
                loadgr();
                loaddata();
            }
